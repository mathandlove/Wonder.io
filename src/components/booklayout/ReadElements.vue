<template>
  <div v-for="linePart in this.ParagraphDisplay" :key="linePart">
    <div
      v-if="linePart.words.length < 1"
      :style="{
        'padding-bottom': this.$store.state.AspectRatio > 2 ? '3.1vh' : '4.4vh',
      }"
    />
    <div v-else-if="linePart.words[0].includes('<im>')">
      <img
        :src="require(`@/assets/Books/book10/images/1.png`)"
        alt=""
        :style="{
          height:
            this.$store.state.AspectRatio > 2
              ? this.imageHeight(linePart.words[0]) * 3.6 + 'vh'
              : this.imageHeight(linePart.words[0]) * 4.4 + 'vh',
          'margin-bottom':
            this.$store.state.AspectRatio > 2 ? '-0.2vh' : '0.2vh',
        }"
      />
    </div>
    <div v-else-if="linePart.words[0].includes('<l>')">
      <div
        ...
        :class="[
          this.$store.state.AspectRatio > 2 ? 'CharPaddingB2' : 'CharPaddingB1',
        ]"
      >
        <img
          :src="
            require(`@/assets/Books/book${
              this.$store.state.BookID
            }/characters/${this.charImageLeft(linePart.words[0])}.png`)
          "
          alt=""
          :class="[
            this.$store.state.AspectRatio > 2 ? 'CharHeight2' : 'CharHeight1',
          ]"
        />
        <div
          class="leftTextPadding"
          :class="[
            this.$store.state.AspectRatio > 2 ? 'TextSize2' : 'TextSize1',
          ]"
        >
    ...
    <div v-else-if="linePart.words[0].includes('<r>')">
      ...
      >
        ...
        <img
          :src="require(`@/assets/Books/book10/characters/captain.png`)"
          alt=""
          :class="[
            this.$store.state.AspectRatio > 2 ? 'CharHeight2' : 'CharHeight1',
          ]"
    ...
    <div v-else>
      <div v-if="linePart.words[0].includes('Moboto')">
        <div
          :class="[
            this.$store.state.AspectRatio > 2 ? 'RobotoSize2' : 'RobotoSize1',
          ]"
  ...
  <div
    v-if="this.ParagraphCounter < this.ParagraphTotal"
  ...
  methods: {
    ...
    imageNumber(inputString) {
      ...
      if (tempNumber.includes("<")) {
        tempNumber = tempNumber.charAt(0);
      }
      return tempNumber;
    },
